...
@section('content')
    <div class="container mx-auto px-6 py-8">
        <h2 class="text-3xl font-bold text-gray-800 mb-6">Create New Promotion</h2>
        ...
        <div class="bg-white shadow-md rounded-xl p-6" x-data="{
            ...
            <form action="{{ route('promos.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" name="name"
                            ...
                            required placeholder="e.g. Ramadan Sale">
                    </div>
                    <div>
                ...
                <div class="mb-6">
                    ...
                    <select name="product_id" class="select2 w-full border-gray-300 rounded-lg shadow-sm">
                        ...
                        @foreach ($products as $product)
                            <option value="{{ $product->id }}">{{ $product->product_name }} - Rp
                                {{ number_format($product->selling_price, 0) }}</option>
                        @endforeach
                    </select>
                    <p class="text-xs text-gray-500 mt-1">Leave empty to apply to specific logic later, currently best to
                        select a product.</p>
                </div>
                ...
                <div x-show="type === 'buy_x_get_y'" class="bg-purple-50 p-4 rounded-lg mb-6 border border-purple-100"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div>
                            ...
                            <input type="number" name="buy_qty" class="w-full border-gray-300 rounded-lg shadow-sm"
                                placeholder="e.g. 2" :disabled="type !== 'buy_x_get_y'">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Get Quantity (Y)</label>
                            <input type="number" name="get_qty" class="w-full border-gray-300 rounded-lg shadow-sm"
                                placeholder="e.g. 1">
                        </div>
                        <div>
                ...
                <div x-show="type === 'bundle'" class="bg-green-50 p-4 rounded-lg mb-6 border border-green-100"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" name="buy_qty" class="w-full border-gray-300 rounded-lg shadow-sm"
                                placeholder="e.g. 3" :disabled="type !== 'bundle'">
                        </div>
                        <div>
                            ...
                            <div class="relative rounded-md shadow-sm">
                                ...
                                <input type="number" name="bundle_price"
                                    class="pl-10 w-full border-gray-300 rounded-lg shadow-sm" placeholder="e.g. 10000">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit"
                        class="px-4 py-2 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition">Save
                        Promotion</button>
                </div>
            </form>
        </div>
